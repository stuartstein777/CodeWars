...
// 	for idx, c := range fight {
// 		if _, ok := LeftPowers[c]; ok {
// 			if ((idx > 0 && fbs[idx-1] != 't') && (idx < l-1 && fbs[idx+1] == 'j')) ||
// 				((idx > 0 && fbs[idx-1] == 'j') && (idx < l-1 && fbs[idx+1] != 't')) {
// 				fbs[idx] = byte(opposites[c])
// 			}
// 		} else if _, ok := RightPowers[c]; ok {
// 			if ((idx > 0 && fbs[idx-1] != 'j') && (idx < l-1 && fbs[idx+1] == 't')) ||
// 				((idx > 0 && fbs[idx-1] == 't') && (idx < l-1 && fbs[idx+1] != 'j')) {
// 				fbs[idx] = byte(opposites[c])
// 			}
// 		}
// 	}
// 	// for idx, c := range fight {
// 	// 	if c == 't' || c == 'j' {
// 	// 		if idx > 0 && fbs[idx-1] != 't' && fbs[idx-1] != 'j' {
// 	// 			prev := rune(fbs[idx-1])
// 	// 			if (c == 't' && (prev == 'm' || prev == 'q' || prev == 'd' || prev == 'z')) ||
// 	// 				(c == 'j' && (prev == 'p' || prev == 'b' || prev == 's' || prev == 'w')) {
// 	// 				fbs[idx-1] = byte(opposites[prev])
// 	// 			}
// 	// 		}
// 	// 		if idx < l-1 && fbs[idx+1] != 't' && fbs[idx+1] != 'j' {
// 	// 			nxt := rune(fbs[idx+1])
// 	// 			if (c == 't' && (nxt == 'm' || nxt == 'q' || nxt == 'd' || nxt == 'z')) ||
// 	// 				(c == 'j' && (nxt == 'p' || nxt == 'b' || nxt == 's' || nxt == 'w')) {
// 	// 				fbs[idx+1] = byte(opposites[nxt])
// 	// 			}
// 	// 		}
// 	// 	}
// 	// }
...
func main() {
	fmt.Println(AlphabetWar("z"))

	//fmt.Println(Score("tsdbtjzddfjtwmbmsabwmjjzjwtfjqdjteww"))
}

//o: tzqjjbwdetwdmzdtzjjmtzp
//a: ts
//e:
...
// run the fight string through the C# version, see what changes.

/*

o: tzqjjbwdetwdmzdtzjjmtzp
a: tzqjjbwdetwdmzdtzjjmtzp
e: tsqjjdwdetwdmzbtzjjmtsp
